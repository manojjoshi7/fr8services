...
class CSRF_Protection
{
  ...
  // -----------------------------------------------------------------------------------
	  ...
	  $hastoken= $this->CI->session->userdata(self::$token_name); 
     if($hastoken=== FALSE || empty($hastoken))
	  {
    ...
    public function validate_tokens()
	{
	  ...
	  if ($_SERVER['REQUEST_METHOD'] == 'POST')
	  {
		...
		if($posted_token === FALSE || empty($posted_token) || $posted_token != $this->CI->session->userdata(self::$token_name))
		{
